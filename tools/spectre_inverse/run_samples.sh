...
GPU_LIST="${GPU_LIST:-1 2 3 4 5 6 7}"   # default GPU set: 0-7
ENV_NAME="${ENV_NAME:-spectre}"               # conda env
SCRIPT="${SCRIPT:-demo_batch.py}"        # Python script
...
for i in $(seq 0 $((NGPUS-1))); do
  ...
  (
    ...
    python "$SCRIPT" --audio --input_dir "$dir" $EXTRA_ARGS
  ) &
done
