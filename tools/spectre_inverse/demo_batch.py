...
# CUDA_VISIBLE_DEVICES=1 python demo_batch.py --input_dir /data/haiyang/wan22_datas/HuMo/Animations/MEAD_25fps/W040/video/front --audio
...
fmt = lambda s: str(Path(*Path(s).parts[:2]) / "#".join(Path(s).parts[2:]))

def extract_frames(video_path, detect_landmarks=True):
    videofolder = os.path.splitext(video_path)[0]
    videofolder = videofolder.replace("/data/haiyang/wan22_datas/HuMo/Animations/MEAD_25fps/", "tmp_MEAD/")
...
def main(args):
    ...
    # /data/haiyang/wan22_datas/HuMo/Animations/MEAD_25fps/M003/video/front/angry/level_1/001.mp4
    video_files = sorted(glob.glob(args.input_dir + "/*/*/*.mp4")) # RAVDESS, with audio
    # import pdb; pdb.set_trace()
    for input_file in video_files:
        try:
            image_paths, landmarks, videofolder, fps = extract_frames(input_file, detect_landmarks=args.crop_face)
            if args.crop_face:
                landmarks = landmarks_interpolate(landmarks)
                if landmarks is None:
                    print('No faces detected in input {}'.format(input_file))
            ...
            with torch.no_grad():
                for chunk_id in range(len(overlapping_indices)):
                    ...
                    all_coeff_exp.append(codedict['exp'].detach().cpu())
                    all_coeff_tex.append(codedict['tex'].detach().cpu())
                    all_coeff_pose.append(codedict['pose'].detach().cpu())
                    all_coeff_light.append(codedict['light'].detach().cpu())
                    all_coeff_shape.append(codedict['shape'].detach().cpu())
            ...
            vid_exp = np.array(torch.cat(all_coeff_exp, dim=0))[2:-2]
            vid_tex = np.array(torch.cat(all_coeff_tex, dim=0))[2:-2]
            vid_pose = np.array(torch.cat(all_coeff_pose, dim=0))[2:-2]
            vid_light = np.array(torch.cat(all_coeff_light, dim=0))[2:-2]
            vid_shape = np.array(torch.cat(all_coeff_shape, dim=0))[2:-2]
            
            save_npz = {"exp": vid_exp,
                        "pose": vid_pose,
                        "tex": vid_tex,
                        "light": vid_light,
                        "shape": vid_shape}
            ...
            videofolder = videofolder.replace("video/", "")
            videofolder = fmt(videofolder)
            

            debug_path = videofolder.replace("tmp_MEAD", "MEAD_v4/debug/")
            coeff_path = videofolder.replace("tmp_MEAD", "MEAD_v4/coeff/")
            audio_path = videofolder.replace("tmp_MEAD", "MEAD_v4/speech/")
            os.makedirs(debug_path.rsplit('/', 1)[0], exist_ok=True)
            os.makedirs(coeff_path.rsplit('/', 1)[0], exist_ok=True)
            os.makedirs(audio_path.rsplit('/', 1)[0], exist_ok=True)

            # import pdb; pdb.set_trace()
            sf.write(audio_path+".wav", wav, sr)
            np.savez(coeff_path + ".npz", **save_npz)
            torchvision.io.write_video(debug_path+".mp4", grid_vid, fps=fps)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i_dir', '--input_dir', default='/data/haiyang/wan22_datas/HuMo/results/prompt_12_3000/', type=str,
                        help='path to the test data, can be image folder, image path, image list, video')
