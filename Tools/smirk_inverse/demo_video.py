...
if __name__ == '__main__':
    ...
    read_index = 0
    read_blink = np.load('/home/haiyang/wan22_data_wheel/Wan2.2/scipts_exps/debug.npy')
    while True:
        ret, image = cap.read()
        ...
        # import random
        # arr = np.array([[round(random.uniform(0.7, 1.0), 8), round(random.uniform(0.7, 1.0), 8)]])
        # outputs['eyelid_params'] = torch.as_tensor(arr, dtype=outputs['eyelid_params'].dtype, device=outputs['eyelid_params'].device)
        # import pdb; pdb.set_trace()
        outputs['eyelid_params'] = torch.as_tensor(np.array([read_blink[read_index]]), dtype=outputs['eyelid_params'].dtype, device=outputs['eyelid_params'].device)
